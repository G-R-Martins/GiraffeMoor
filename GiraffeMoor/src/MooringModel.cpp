	...
	for (SegmentProperty& seg : segment_property_vector)
	{
		...
		if (seg.type == 'b' && seg.E != 0)
		{
			if (seg.SG != 1.0 && seg.E > 0.0)
				seg.diameter = sqrt(4 * seg.rho / (PI * seg.SG * environment.rhofluid));
...
bool MooringModel::GenerateCatenary()
{
	...
	for (Line& line : line_vector)
	{
		...
		{
			unsigned int n_nodes = line.segments[seg].n_nodes;
										   ...
										   double& Hf, double& Vf, Matrix& Fairleads_StiffnessMatrix)
{
	...
	for (unsigned int seg = 0; seg < n_segs; seg++)
	{
		line.total_length += line.segments[seg].length;

		//Gamma_s
		line.gamma_s.emplace_back(g * segment_property_vector[line.segments[seg].property - 1].rho
								  - g * environment.rhofluid * PI * pow(segment_property_vector[line.segments[seg].property - 1].diameter, 2) / 4.0);
	}
	...
	A(0, 0) = keypoint_vector[point_A - 1].x;
	A(1, 0) = keypoint_vector[point_A - 1].y;
	A(2, 0) = keypoint_vector[point_A - 1].z;
	B(0, 0) = keypoint_vector[point_B - 1].x;
	B(1, 0) = keypoint_vector[point_B - 1].y;
	B(2, 0) = keypoint_vector[point_B - 1].z;

	//Calculating original position of the fairlead to calculate analytical stiffness matrix
	Fairleads_StiffnessMatrix(0, 0) = keypoint_vector[point_B - 1].x;
	Fairleads_StiffnessMatrix(1, 0) = keypoint_vector[point_B - 1].y;
	Fairleads_StiffnessMatrix(2, 0) = keypoint_vector[point_B - 1].z;
}
	...
	for (aux0 = 1; aux0 <= 2; aux0++)
	{
		...
		for (aux1 = 1; aux1 < MAX_ITERATIONS_FH0; aux1++)
		{
			...
			for (aux2 = 1; aux2 < MAX_ITERATIONS_FV0; aux2++)
			{
				...
				for (aux3 = 0; aux3 <= 10; aux3++)
				{
					...
					{
						dif -= line.segments[cont_ds].length * line.gamma_s[cont_ds];
		...
		else if (aux0 == 1)
		{
			...
			for (unsigned int seg = 0; seg <= lim_seg; seg++)
			{
				...
				elements += line.segments[seg].discretization;
			}
			...
			B(2, 0) = keypoint_vector[line.keypoint_B - 1].z - dz;
		}
	}
...
void MooringModel::SetLinesConfiguration(Line& line, Matrix& F, std::vector <double>& FV, const unsigned int& n_segs)
{
	...
	else
	{
		...
		for (unsigned int i = 0; i <= seg_tdp; i++)
			sum_len_tdp += line.segments[i].length;
		...
		for (unsigned int seg = 0; seg <= seg_tdp; seg++)
			leng_ext += line.segments[seg].length * ( 1 + F(0, 0) / segment_property_vector[line.segments[seg].property - 1].EA );

		x_tdp_ext = leng_ext + leng_tdp * ( 1 + F(0, 0) / segment_property_vector[line.segments[seg_tdp].property - 1].EA );
	}
...
void MooringModel::GenerateCatenaryTDZ(Line& line, const unsigned int& n_segs, unsigned int& seg_init)
{
	...
	if (line.tdz->x_a < 0)
	{
		...
		line.tdz->tdz_a.len_elem_out = line.segments[0].length / line.segments[0].discretization;
	}
	...
	if (line.tdz->x_f > line.total_length)
	{
		...
		line.tdz->tdz_f.len_elem_out = line.segments[n_segs - 1].length / line.segments[n_segs - 1].discretization;
	}
	...
	{
		cur_x += line.segments[cur_seg].length;
	...
	if (seg_tdp - line.tdz->tdz_a.seg > 1)
	{
		...
		line.tdz->tdz_a.len_elem_out = line.segments[seg_tdp - 1].length / line.segments[seg_tdp - 1].discretization;
	}
	else
		line.tdz->tdz_a.len_elem_out = line.segments[seg_tdp].length / line.segments[seg_tdp].discretization;
	if (line.tdz->tdz_f.seg - seg_tdp > 1)
	{
		line.tdz->tdz_f.node_extreme_seg = true;
		line.tdz->tdz_f.seg = ( int )seg_tdp;
		line.tdz->tdz_f.len_elem_out = line.segments[( size_t )seg_tdp + 1].length / line.segments[( size_t )seg_tdp + 1].discretization;
	}
	else
		line.tdz->tdz_f.len_elem_out = line.segments[seg_tdp].length / line.segments[seg_tdp].discretization;
	...
	if (line.tdz->tdz_a.seg > 0)
	{
		...
		{
			len_before_seg_a += line.segments[seg].length;
			line.tdz->tdz_a.elem += line.segments[seg].discretization;
	...
	double len = line.segments[seg_tdp].length / line.segments[seg_tdp].discretization;
	...
	{
		line.tdz->tdz_a.elements_out = ( unsigned int )line.segments[( size_t )seg_tdp - 1].length / line.segments[( size_t )seg_tdp - 1].discretization;
		line.tdz->tdz_a.node_extreme_seg = true;
	...
	if (line.tdz->x_f > len_before_seg_a + line.segments[seg_tdp].length && !line.tdz->tdz_f.node_extreme_seg)
	{
		line.tdz->tdz_f.elements_out = ( unsigned int )line.segments[( size_t )seg_tdp + 1].length / line.segments[( size_t )seg_tdp + 1].discretization;
		line.tdz->tdz_f.node_extreme_seg = true;
		line.tdz->x_f = len_before_seg_a + line.segments[seg_tdp].length;
	}
	else //Caso contrário, acha quantidade de elementos no segmento após a TDZ
	{
		line.tdz->tdz_f.elements_out = ( unsigned int )floor(( len_before_seg_a + line.segments[seg_tdp].length - line.tdz->x_f ) / len);
		line.tdz->x_f = len_before_seg_a + line.segments[seg_tdp].length - line.tdz->tdz_f.elements_out * len;
	}
	...
	int n = segment_property_vector[line.segments[seg_tdp].property - 1].type == 't' ? 1 : 2;
...
void MooringModel::SetMeshProperties(Line& line, const unsigned int& n_segs)
{
	...
	{
		size_t div = (size_t)line.segments[seg].discretization;
		double len = line.segments[seg].length;
		...
		for (size_t i = 1; i <= div; i++)
		{
			...
			if (segment_property_vector[line.segments[seg].property - 1].type == 't')
	...
	if (TDZ)
	{
		...
		size_t div = line.segments[seg_tdp].discretization;
		double len = line.segments[seg_tdp].length;
		if (!line.tdz->tdz_a.node_extreme_seg)
		{
			for (size_t i = 1; i <= (size_t)line.tdz->tdz_a.elements_out; i++)
			{
				if (segment_property_vector[line.segments[seg_tdp].property - 1].type == 't')
		...
		if (line.tdz->tdz_f.seg > n_segs - 1)
		{
			...
			{
				div = line.segments[seg].discretization;
				len = line.segments[seg].length / div;
...
void MooringModel::GenerateMesh(Line& line, Matrix& A, Matrix& F, double& Hf, double& Vf)
{
	...
	for (size_t seg = 0; seg < line.segments.size(); seg++)
	{
		...
		nodes_line += line.segments[seg].n_nodes; 

		char type = segment_property_vector[line.segments[seg].property - 1].type;
		int n = (type == 't') ? 1 : 2;
		...
		if (seg > 0)
		{
			if ((segment_property_vector[line.segments[seg - 1].property - 1].type == 't' && segment_property_vector[line.segments[seg].property - 1].type == 'b' && line.segments.size() >= 2))
				line.transition_nodes.push_back(cur_node_mesh - 1);
		}

		//Generate nodes, nodesets and elements
		for (size_t cur_elem_seg = 1; cur_elem_seg <= line.segments[seg].n_elements; cur_elem_seg++)
		{
			...
			//Fairlead node
			if (seg == ( line.segments.size() - 1 ) && cur_elem_seg == ( size_t )line.segments[seg].n_elements)
			{
			...
			//Generate elements
			if (type == 't')
				gm.GenerateTrussElement(cur_elem, bool_first_element, line.segments[seg].property, cur_node_mesh - 2, cur_node_mesh - 1);
			else //type == 'b'
				gm.GeneratePipeElement(cur_elem, bool_first_element, line.segments[seg].property, cur_cs, cur_node_mesh - 3, cur_node_mesh - 2, cur_node_mesh - 1);
			++cur_elem;
		}	
	}
												...
												std::vector<std::vector<double>>& xcat_n, std::vector<std::vector<double>>& zcat_n, std::vector<std::vector<double>>& roty_n)
{
	...
	for (unsigned int seg = 0; seg < n_segs; seg++)
	{
		double EA_n = segment_property_vector[line.segments[seg].property - 1].EA;

		for (unsigned int node = 0; node < line.segments[seg].n_nodes; node++)
		{
			if (seg > 0 && node == 0)
				sum_len += line.segments[seg - 1].length;
	...
	for (unsigned int seg = 0; seg < x0_n.size(); seg++)
	{
		...
		for (unsigned int node = node_init; node < line.segments[seg].n_nodes; node++)
		{
	...
	size_t node_rot = line.segments[seg_rot].n_nodes - 1;
...
void MooringModel::CheckDummyElements()
{
	...
	double dummy_z = environment.waterdepth; //depth -> dummy elements are generated under the seabead
...
//	if (stiff_matrix && stiff_matrix->bool_num && environment.seabed.mu > 0)
//	{
...
//		bool_c2.Push_Back(false);
//
//		gm.GenerateNSSSContact(1, node_set_contact, 1, environment.seabed.mu, environment.seabed.stiffness, environment.seabed.damping, environment.seabed.stiffness * 0.1, 0.0, environment.seabed.pinball, environment.seabed.radius, 1, bool_c);
//		//gm.GenerateNSSSContact(2, node_set_contact, 1, 0.0, environment.seabed.stiffness, environment.seabed.damping, environment.seabed.stiffness * 0.1, 0.0, environment.seabed.pinball, environment.seabed.radius, 1, bool_c2);
//	}
//	else
//	{
//		bool_c.Set(1, true);
...
//void MooringModel::MoveLines2Vessels()
//{
//	//Moves lines to vessels objects
//	for (Line& line : line_vector)
//	{
//		int vessel_ID = line.vessel - 1;
//		
//		/*Case with a vessel connection -> move to 'vessel_vector[vessel_number].line_vector' and delete it
//			Otherwise	-> keep in the (mm) line_vector */
//		if (vessel_ID >= 0)
//		{
//			//vessel_vector[( size_t )vessel_ID].line_vector.emplace_back(std::move(line));
//		}
//	}
//
//	//Removes lines from 'line_vector'
//	line_vector.erase(std::remove_if(line_vector.begin(), line_vector.end(),
//									 [&](const Line& line) { return line.vessel > 0; }), line_vector.end());
//};
...
void MooringModel::GenerateDynamicRelaxation()
{
	...
	{
		double temp_ma = environment.rhofluid * area_eq[line];
		if (temp_ma > m_a)
		{
			m_a = environment.rhofluid * area_eq[line];
			m_m = rho_eq[line];
			q = ( m_m - m_a ) * g;
		}
...
void MooringModel::GenerateVessel()
{
	...
	for (Vessel& vessel : vessel_vector)
	{
		...
		std::array<double, 3> vessel_coord{ keypoint_vector[vessel.keypoint - 1].x, keypoint_vector[vessel.keypoint - 1].y, keypoint_vector[vessel.keypoint - 1].z };

		sprintf(comment, "Vessel number %d", vessel.number);
		//Node
		gm.GenerateNode(cur_node_mesh, vessel_coord, comment);
		vessel.node = cur_node_mesh;

		//Node Set (vessel)
		vessel.nodeset = cur_node_set;
		gm.GenerateNodeSet(vessel.nodeset, cur_node_mesh, comment);
		++cur_node_set;
		...
		if (platform_vector.empty())
		{
			...
			gm.GenerateRigidBodyElement(vessel.element, cur_rbdata, 1, cur_node_mesh++);
			++cur_elem;	
			...
			auto it = std::find(line_vector.begin(), line_vector.end(), vessel.number);
			while (it != line_vector.end() && it->vessel == vessel.number)
			{
				vessel.nodes_rigid_nodeset.emplace(cur_node_mesh);
				gm.GenerateNode(cur_node_mesh, keypoint_vector[it->keypoint_B - 1].x, keypoint_vector[it->keypoint_B - 1].y, keypoint_vector[it->keypoint_B - 1].z);
				gm.GenerateSameDisplacement(++cur_special_constraint, cur_node_mesh++, it->node_B, bool_t);
				
				++it;
			}
			vessel.rigid_nodeset = cur_node_set++;
			
//			/*If exist shared line(s), must check if the fairleads must be included to some rigid node set
//				if is already included, do nothing */
//			sprintf(comment, "Vessel Rigid Node Set");
//			gm.GenerateNodeSet(cur_node_set, vessel.rigid_nodeset, comment);
//			bool_t.Set(1, true);
//			gm.GenerateRigidNodeSet(++cur_special_constraint, vessel.node, cur_node_set++, bool_t);
		}
...
//				platform.VesselRigidNodeset(cur_node_set, cur_special_constraint, vessel_vector);
		...
		if (gm.monitor.bool_nodes_vessel)
			gm.monitor.nodes.push_front(vessel.node);
		if (gm.monitor.bool_elements_vessel)
			gm.monitor.elements.push_front(vessel.element);
	}
...
void MooringModel::GenerateRigidNodeSets()
{
	...
	if (platform_vector.empty())
	{
		...
		{
			sprintf(comment, "Vessel Rigid Node Set");
			gm.GenerateNodeSet(vessel.rigid_nodeset, vessel.nodes_rigid_nodeset, comment);
			gm.GenerateRigidNodeSet(++cur_special_constraint, vessel.node, vessel.rigid_nodeset, bool_t);
		}
	}
	else
	{
...
void MooringModel::IncludeSharedLinesFaileads(BoolTable& bool_t)
{
	...
	{
		//Booleans to indicate if the nodes were included to some rigid nodeset
		//bool coupled_nodeA = false, coupled_nodeB = false;

		//Check if the extremities nodes is already in some rigid nodeset
		for (Vessel& vessel : vessel_vector)
		{
			auto& rigid_NS = vessel.nodes_rigid_nodeset;
			...
			if (std::find(rigid_NS.begin(), rigid_NS.end(), line_it->node_A) == rigid_NS.end())
			{
				...
				if (line_it->vesselIDs[0] == vessel.number)
				{
					rigid_NS.emplace(cur_node_mesh); //Include to vessel rigid nodeset

					sprintf(comment, "Node to couple line %d and vessel %d", line_it->number, vessel.number);

					gm.GenerateNode(cur_node_mesh, keypoint_vector[line_it->keypoint_A - 1].x, keypoint_vector[line_it->keypoint_A - 1].y, keypoint_vector[line_it->keypoint_A - 1].z, comment);
			...
			{
				//Check if must be included
				if (line_it->vesselIDs[1] == vessel.number)
				{
					rigid_NS.emplace(cur_node_mesh); //Include to vessel rigid nodeset

					sprintf(comment, "Node to couple line %d and vessel %d", line_it->number, vessel.number);

					gm.GenerateNode(cur_node_mesh, keypoint_vector[line_it->keypoint_B - 1].x, keypoint_vector[line_it->keypoint_B - 1].y, keypoint_vector[line_it->keypoint_B - 1].z, comment);
...
*/
	...
	if (environment.seacurrent_exist)
	{
		...
		for (int cont = 0; cont < seacurrent_booltable; cont++)
			gm.environment.bool_current.Push_Back(false);
		gm.environment.bool_current.Push_Back(true);
	...
	//Sea current bool table, just for Morison effects
		...
		gm.environment.bool_current.Set(3, false, false, true);
...
void MooringModel::GenerateVesselDisplacements(unsigned int& step)
{
	for (VesselDisplacement& disp : vessel_disp_vector)
	{
		...
		else if (disp.isTable)
		{
			...
			if (stiff_matrix && stiff_matrix->bool_num)
				i = environment.seacurrent_exist ? 26 : 25;
...
void MooringModel::GenerateForces()
{
	for (MoorLoad& load : moorload_vector)
	{
			...
			for (unsigned int seg = 0; seg < load.segmentID; ++seg)
			{
				...
				if (load.segmentID == seg + 1)
				{
					if (load.description == "first")
						add_nodes = 1;
					else if (load.description == "last")
						add_nodes = nodes_seg;
					else if (load.description == "middle")
		...
		{
			for (int i = 0; i < ( int )load.table->GetLines(); i++)
...
void MooringModel::GenerateSeabed()
{
	...
	for (const Line& moor : line_vector)
	{
		...
		//Check X (min e max)
		if (keypoint_vector[pointA].x < xmin)	xmin = keypoint_vector[pointA].x;
		if (keypoint_vector[pointB].x < xmin)	xmin = keypoint_vector[pointB].x;
		if (keypoint_vector[pointA].x > xmax)	xmax = keypoint_vector[pointA].x;
		if (keypoint_vector[pointB].x > xmax)	xmax = keypoint_vector[pointB].x;

		//Check Y (min e max)
		if (keypoint_vector[pointA].y < ymin)	ymin = keypoint_vector[pointA].y;
		if (keypoint_vector[pointB].y < ymin)	ymin = keypoint_vector[pointB].y;
		if (keypoint_vector[pointA].y > ymax)	ymax = keypoint_vector[pointA].y;
		if (keypoint_vector[pointB].y > ymax)	ymax = keypoint_vector[pointB].y;
	}
	...
	//Pinball
	environment.seabed.pinball = (xmax - xmin);
	//Radius to search contact
	environment.seabed.radius = 0.0;

	//Pilot node (seabed)
	std::array<double, 3> pilot_p = { ( xmax + xmin ) / 2.0, ( ymax + ymin ) / 2.0, -environment.waterdepth };
	...
	environment.seabed.pilot_node = pil;
	...
	{
		VTKseabedOk = gm.post.CreateSeabedVTK(surfaces_folder, { 2.0 * xmin, 2.0 * fabs(xmax) }, { 2.0 * ymin, 2.0 * fabs(ymax) }, -environment.waterdepth);
...
void MooringModel::GenerateConstraints()
{
	...
	else
	{
		...
		if (constrainted_vessels.size() != vessel_vector.size())
		{
			...
			for (Vessel& vessel : vessel_vector)
			{
				if (constrainted_vessels.find(vessel.number) != constrainted_vessels.end())
					continue; //found -> already constrainted
				else
				{
					constrainted_vessels.insert(vessel.number);
					gm.GenerateNodalConstraint(++cur_constraint, vessel.nodeset, U, U, U, ROT, ROT, ROT);
				}
			}
		}
