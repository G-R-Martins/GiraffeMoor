...
std::unordered_map<std::string_view, std::function<bool(std::string& readed)>> IO::s_func_reading = {
	{ "Keypoint", &IO::ReadKeypoint }, 

	{ "Set", &IO::ReadSegmentSet }, { "Length", &IO::ReadSegment }, 
	
	{ "Line", &IO::ReadLine }, 

	{ "Vessel", &IO::ReadVessel }, 
	
	{ "SegmentProperty", &IO::ReadSegmentProperty}, 
	
	{ "General", &IO::ReadEnvGeneral}, { "Seabed", &IO::ReadSeabed}, 
	{ "SeaCurrent", &IO::ReadSeaCurrent}, { "Depth", &IO::ReadSeaCurrentAt }, 

	{ "DynamicRelaxation", &IO::ReadDynamicRelaxation},
	{ "LineStatics", &IO::ReadDynRelaxLines }, { "VesselStatics", &IO::ReadDynRelaxVessels}, 
	{ "SeaCurrentStep", &IO::ReadSeaCurrentStep}, 
	{ "Analysis", &IO::ReadAnalysis}, { "Step", &IO::ReadSolutionStep},

	{ "Run", &IO::ReadRunOption }, { "Processors", &IO::ReadProcessors },
	{ "ConvergenceCriteria", &IO::ReadConvergenceCriteria },

	{ "PostFiles", &IO::ReadPostFiles }, { "CADs", &IO::ReadCADs },

	{ "Analytical", &IO::ReadAnalyticalStiffnessMatrix }, { "CADs", &IO::ReadNumericalStiffnessMatrix },

	{ "Vessel", &IO::ReadVesselDisplacement }
};
...
bool IO::ReadFile()
{
	...
	while (s_inp.good())
	{
		...
		if (!aux_read::CheckTopKeyword(word))
		{
			...
			if (aux_read::TryCommentAndContinue(s_inp, word))
				continue;
		...
		if (word == "Keypoints")
		{
			if (!aux_read::Loop(s_inp, mm.keypoint_vector, USET_SV{ "Keypoint" }, word, s_func_reading))
		...
		{
			if (!aux_read::Loop(s_inp, mm.segment_set_vector, USET_SV{ "Set" }, word, s_func_reading))
		...
		{
			if (!aux_read::Loop(s_inp, mm.line_vector, USET_SV{ "Line", "Cable" }, word, s_func_reading))
		...
		{
			if (!aux_read::Loop(s_inp, mm.vessel_vector, USET_SV{ "Vessel" }, word, s_func_reading))
		...
		{
			if (!aux_read::UniqueSubKeywords(s_inp, mm.environment,
				USET_SV{ "General", "Seabed", "SeaCurrent" }, word, s_func_reading))
				std::cout << "\nErro lendo Environment! Linha " << aux_read::GetCurrentLine(s_inp) << "\n"; // TODO: verificar erro leitura
		}
		else if (word == "SegmentProperties")
		{
			if (!aux_read::Loop(s_inp, mm.segment_property_vector, USET_SV{ "SegmentProperty" }, word, s_func_reading))
		...
		{
			if (!aux_read::UniqueSubKeywords(s_inp, mm.moorsolution,
				USET_SV{ "DynamicRelaxation", "SeaCurrentStep", "Analysis" }, word, s_func_reading))
		...
		{
			if (!aux_read::UniqueSubKeywords(s_inp, gm.gir_solver,
				USET_SV{ "Run", "Processors", "ConvergenceCriteria" }, word, s_func_reading))
				std::cout << "\nErro lendo Solution! Linha " << aux_read::GetCurrentLine(s_inp) << "\n"; // TODO: verificar erro leitura
		}
		else if (word == "PostProcessing")
		{
			if (!aux_read::UniqueSubKeywords(s_inp, mm.moorpost,
				USET_SV{ "PostFiles", "CADs" }, word, s_func_reading))
...
bool IO::ReadSegmentSet(std::string& readed)
{
	...
	if (!aux_read::NestedLoop(s_inp, set->GetAllSegment(), 
		names, upper_keywords, readed, s_func_reading))
...
{
	if (!aux_read::UniqueSubKeywords(s_inp, mm.moorsolution,
		USET_SV{ "LineStatics", "VesselStatics"}, readed, s_func_reading))
	{
...
bool IO::ReadDynRelaxLines(std::string& readed)
{
	...
	relax = std::make_unique<DynRelaxLinesConfig>(0.0, 0, 0, 0, false, true, 0.0, 0.0, 1.0, 1.0e-1, 1.0e-4, 1'000'000,
		0.0, 0.0, 0.0, 0.0);
...
bool IO::ReadDynRelaxVessels(std::string& readed)
{
	...
	relax = std::make_unique<SolutionStep>(0, false, true, 0.0, 0.0, 1.0, 1.0e-1, 1.0e-4, 1'000'000,
		0.0, 0.0, 0.0, 0.0);
...
{
	std::unordered_set<std::string_view> names = { "Step" };
...
bool IO::ReadSolutionStep(std::string& readed)
{
	...
	//std::unordered_set<std::string_view> to_ignore; // just read keyword and value and ignore with warning
...
{
	std::unordered_set<std::string_view> names = { "Depth" };
...
bool IO::ReadConvergenceCriteria(std::string& readed)
{
	...
	do {
		...
		if (nh.empty())
		{
			bool all_parameters_readed = names.empty();
			if (!all_parameters_readed)
				Log::AddWarning(Log::Warning::Invalid_ID, VEC_STR{ "ConvergenceCriteria", std::to_string(aux_read::GetCurrentLine(s_inp)) });
...
bool IO::ReadPostFiles(std::string& readed)
{
	...
	do {
		...
		if (nh.empty())
		{
			bool all_parameters_readed = names.empty();
			if (!all_parameters_readed)
				Log::AddWarning(Log::Warning::Invalid_ID, VEC_STR{ "PostFiles", std::to_string(aux_read::GetCurrentLine(s_inp)) });
...
{
	std::unordered_set<std::string_view> names = { "PlatformID" };
	std::unordered_set<std::string_view> upper_keywords = { "PostFiles" };

	if (!aux_read::NestedLoop(s_inp, mm.moorpost.GetAllPlatformCADs(),
		names, upper_keywords, readed, s_func_reading))
...
void IO::ReadDisplacementFields(std::string& readed)
{

}
void IO::ReadConstraints(std::string& readed)
{
